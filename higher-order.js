...
//Code Here
let totalPopulation = populations.reduce((total, element) => total + element, 0) // 0 is the starting total 
...
//Code Here
// let myStrongest  = monstersInYourPocket.filter(function(val) {
//   return val["CP"] > 200;
// });
...
//let orderTotals = orders.map((element) => element["price"] + element["tax"])//
...
*/
// I want to get ONLY Bob's purchases. If Bob purchased it, callback a function to add that price to bobsTotal.
// let bobsTotal = purchases.map((element) => element["owner"] === "Bob" ? element["price"]
let bobsTotal = purchases.reduce( function(total, val){
  if (val["owner"] === "Bob") {
    return total + val["price"];
  } else {
    return total; // do not update total if it's not Bob
  }
}, 0)
